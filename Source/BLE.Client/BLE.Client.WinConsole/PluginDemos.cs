	...
	internal class PluginDemos
	{
		...
		private async Task Bond(PairModes pairModes)
		{
			...
			if (_adapter is IBondable bondable)
			{
				...
				try
				{
					var bondResult = await bondable.BondAsync(_selectedDevice, new(pairModes, ProtectionLevel.None), cts.Token);
		...
		private async Task DoTheScanning(ScanMode scanMode = ScanMode.LowPower, int timeMs = 5000)
		{
